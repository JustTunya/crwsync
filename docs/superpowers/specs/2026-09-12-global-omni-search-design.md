...
**Shared highlight utility** — `hooks/use-highlight-target.ts`: `highlightTarget(id: string, flashSelector?: string)` → `document.getElementById(id)?.scrollIntoView({behavior:"smooth", block:"center"})` + add `bg-primary/20` to the target itself, or to a nested element matched by `flashSelector`, for ~1.5s. Used by chat/file/member deep-link handlers below (3 real call sites — genuine dedup, not speculative). Chat reuses the existing, already-working reply-jump convention exactly: `MessageBubble.tsx`'s root sets `id={"message-" + message.id}` and its reply-jump handler already flashes a nested `.message-highlight-target` element — the search deep-link targets the same `message-` + id and the same nested selector, no fix needed there.
...
- **Chat message** → `router.push("/" + slug + "/chat/" + roomId + "?messageId=" + id)`. `ChatRoom`: if `document.getElementById("message-" + id)` exists, `highlightTarget` immediately. Else (message predates the initial load) call the existing `getChatMessages(workspaceId, roomId, undefined, 50, "before")` from `chat.service.tsx` — same call `useChatMessages` already makes for the initial load — prepend the page into `useChatStore`, then `highlightTarget` on next tick. (The search result's `created_at` isn't threaded through the URL, so this re-fetches the most recent page rather than anchoring on a specific cursor — sufficient since search results are already recent by construction.)
