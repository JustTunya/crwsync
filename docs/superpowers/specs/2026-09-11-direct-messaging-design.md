...
Unread badge: the existing `chat:unread_increment` broadcast (`chat.gateway.ts:240`, emitted workspace-wide on `StatusGateway`) gains one field — `isDirect: boolean` — read from `client.data.currentRoom`'s cached `is_direct` (already fetched in `handleJoinRoom`, stash on `client.data` alongside `currentRoom`). `l-sidebar.tsx`'s existing listener ignores the new field (module-keyed, DMs have no module) — no change needed there. `RSidebar.tsx` adds its own listener on the same `chat:unread_increment` event: if `isDirect` and `senderId` matches a rendered member, mark that member unread in local state. Initial unread state on mount/refresh comes from `listDms`'s per-room `unreadCount` (`ChatReadReceipt.last_read_at` vs latest message). Since the member-list click is the only way to open a DM, `useOpenDirectMessage`'s `onSuccess` both navigates and invalidates `dmKeys.list(workspaceId)` (same invalidate-on-mutation pattern as `useCreateChatRoom`), clearing that member's dot — no dependency on the `mark_as_read` socket flow or `ChatRoom.tsx` internals.
