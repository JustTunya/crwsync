...
**Two more surfaces found by final review, not in the original four**: (1) `handleSendMessage`'s `@everyone`/`@mentionedUserIds` fan-out (`chat.gateway.ts:242-288`) *pushes* message content to `mention_notification` recipients rather than authorizing a *read*, so it never appeared in the read-path grep above — it must skip entirely (or clamp its recipient set to the two DM participants) when `client.data.currentRoomIsDirect` is true; `ChatInput.tsx`'s `@everyone` option should also be hidden for `is_direct` rooms so the UI doesn't offer what the server refuses. (2) `workspace.service.ts`'s `getTaskAttachmentDownloadUrl` authorizes a chat attachment by workspace membership only (its own comment claims parity with `presignAttachment`, which is no longer true post-fix) — needs the same `is_direct`/participant check as the four original spots.
...
Unread badge: `chat:unread_increment` (`chat.gateway.ts:240`) carries `isDirect: boolean` for DM rooms, read from `client.data.currentRoomIsDirect` (stashed in `handleJoinRoom`). Post-final-review correction: this event and `dm:room_created` are scoped to `user_${dmUserAId}`/`user_${dmUserBId}` (the two participants), not broadcast workspace-wide — the original workspace-wide broadcast leaked DM metadata (who's messaging whom) to every member and caused false unread dots on unrelated members sharing a DM partner. `RSidebar.tsx`'s listener matches on `dm.room.id === roomId` (not `senderId`), and skips the update while the user is already viewing that room. Initial unread state on mount/refresh comes from `listDms`'s per-room `unread: boolean` (`ChatReadReceipt.last_read_at` vs latest message). Since the member-list click is the only way to open a DM, `useOpenDirectMessage`'s `onSuccess` patches `dmKeys.list(workspaceId)` via `setQueryData` (cache-patch-first per house convention) before navigating, clearing that member's dot.
